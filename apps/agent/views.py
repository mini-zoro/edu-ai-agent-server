...
from apps.agent.serializers import (
    ...
    ChatRequestSerializer, AgentTagSerializer
)
...
class AgentView(APIView):
    ...
    def _check_admin_permission(self, request):
        ...
        检查管理员权限
        """
        if not hasattr(request.user, 'type') or request.user.type != 0:
            return R.fail(ResponseEnum.NOT_PERMISSION, "仅管理员可访问此接口")
        return None
    ...
    def get(self, request):
        ...
        #3. 根据用户权限过滤智能体
        queryset = self._filter_agents_by_user_permission(queryset, request.user)
        # 4. 搜索过滤
        if search:
    ...
    def post(self, request):
        ...
        创建智能体（仅管理员可操作）
        """
        permission_check = self._check_admin_permission(request)
        if permission_check:  # 添加权限检查逻辑
    ...
    def delete(self, request):
        ...
        删除智能体（仅管理员可操作）
        """
        permission_check = self._check_admin_permiission(request)
        if permission_check:  # 添加权限检查逻辑
            return permission_check
        agent_id = request.data.get('agent_id')  # 参数验证阶段
        if not agent_id:
